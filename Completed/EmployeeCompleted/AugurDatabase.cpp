...
SaveResult AugurDatabase::save_employee(const Employee& employee)
{
    ...
    if (auto* regular_payment_scheme{
        ...
        augur_db->store_field("overtime"s,
                              regular_payment_scheme->get_overtime_hours());
    } else if (auto* commissioned_payment_scheme{
                       dynamic_cast<const CommissionedPaymentScheme*>(
                               &payment_scheme)}) {
        augur_db->store_field(
                "project"s,
                commissioned_payment_scheme->get_project().get_name());
    } else if (auto* freelance_payment_scheme{
                       dynamic_cast<const FreelancePaymentScheme*>(
                               &payment_scheme)}) {
        augur_db->store_field("billable_hours"s,
                              freelance_payment_scheme->get_regular_hours());
    } else {
